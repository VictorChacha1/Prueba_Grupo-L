...
void buscarOrden() {
    ...
    if (!encontrado) cout << "No se encontraron ordenes." << endl;
}
...
void jugarAhorcado() {
    ...
    srand(time(0));
    string palabra;
    if (nivel == 1) palabra = palabrasFacil[rand() % 5];
    else palabra = palabrasDificil[rand() % 5];
    string letrasUsadas = "";
    int errores = 0;
    int maxErrores = 6;
    bool gano = false;
    ...
    while (errores < maxErrores) {
        ...
        for (int i = 0; i < (int)palabra.length(); i++) {
            bool revelada = false;
            for (int j = 0; j < (int)letrasUsadas.length(); j++) {
                if (letrasUsadas[j] == palabra[i]) { revelada = true; break; }
            }
        ...
        bool repetida = false;
        for (int i = 0; i < (int)letrasUsadas.length(); i++) {
        ...
        if (!acierto) { errores++; cout << "Letra incorrecta." << endl; }
        else cout << "Letra correcta." << endl;
    }
    mostrarAhorcado(errores);
    string ganador;
    if (gano) {
...
void menuPrincipal() {
    ...
    do {
        ...
        switch (opcion) {
            ...
            case 6: cout << "\nTotal facturado: $" << sumarTotalOrdenes(0) << endl; break;
